...
EAPI=6
inherit autotools eutils linux-info multilib user systemd savedconfig
...
"
IUSE="${IUSE_VOICEMAIL_STORAGE} alsa bluetooth calendar +caps cluster curl dahdi debug doc freetds gtalk http iconv ilbc xmpp ldap libedit libressl lua mysql newt opus +samples silk siren14 siren7 odbc osplookup oss pjproject portaudio 
postgres radius selinux snmp span speex srtp static syslog vorbis"
...
RDEPEND="${CDEPEND}
	opus? ( media-libs/asterisk-14-opus-bin )
	selinux? ( sec-policy/selinux-asterisk )
	silk? ( media-libs/asterisk-14-silk-bin )
	siren7? ( media-libs/asterisk-14-siren7-bin )
	siren14? ( media-libs/asterisk-14-siren14-bin )
...
src_configure() {
	restore_config menuselect.makeopts
...
src_install() {
	save_config menuselect.makeopts
...
pkg_postinst() {
	...
	echo
	echo
	elog "Please read the Asterisk 15 upgrade document:"
	elog "https://wiki.asterisk.org/wiki/display/AST/Upgrading+to+Asterisk+15"
}
